...
window.QA_CORE.TcSync = {
    // 🚨 실제 사용하는 구글 스프레드시트의 URL과 하단 탭의 GID 값을 입력해주세요.
    ...
    async fetchAndCountExecution() {
        try {
            // 브라우저 캐시 무력화 (항상 최신 데이터를 가져오기 위함)
            ...
            // 타겟 상태 키워드 (오탐지 방지를 위해 대문자로 강제 비교)
            const targetStatus = ["PASS", "FAIL", "N/A", "BLOCK"];

            // 헤더(1행) 제외하고 순회
            for (let i = 1; i < rows.length; i++) {
                const row = rows[i];
                // 1인 검증 체제: 행 내부 셀에 목표 키워드 중 하나라도 일치하면 1건 카운트
